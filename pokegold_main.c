...
#include "assets_manifest_pokegold.h"
extern uint8_t pokegold__rom_data[];
extern const size_t pokegold__rom_size;
static const GBGameAssets GB_POKEGOLD_GAME = {
    .game_id = "pokegold",
    .rom_filename = "roms/pokegold.gbc",
    .rom_data = pokegold__rom_data,
    .rom_size = 2097152u,
    .expected_sha1 = { 0xd8, 0xb8, 0xa3, 0x60, 0x0a, 0x46, 0x53, 0x08, 0xc9, 0x95, 0x3d, 0xfa, 0x04, 0xf0, 0x08, 0x1c, 0x05, 0xbd, 0xcb, 0x94 },
    .manifest = POKEGOLD_ASSETS_MANIFEST,
    .manifest_count = POKEGOLD_ASSETS_MANIFEST_COUNT,
};
...
int pokegold_main(int argc, char* argv[]) {
    ...
    GBConfig runtime_config = *pokegold_default_config();
    ...
    if (differential_mode) {
        ...
        gb_context_set_save_id(generated_ctx, "pokegold");
        gb_context_set_save_id(interpreted_ctx, "pokegold");
        pokegold_init(generated_ctx);
        pokegold_init(interpreted_ctx);
    ...
    gb_context_set_save_id(ctx, "pokegold");
    ...
    if (!gb_chdir_to_exe_dir()) {
        fprintf(stderr, "[pokegold] chdir to exe dir failed\n");
        return 1;
    }
    if (!gb_load_assets(&GB_POKEGOLD_GAME)) {
        fprintf(stderr, "[pokegold] failed to load ROM/assets\n");
        return 1;
    }
#ifdef GB_HAS_SDL2
    gb_platform_set_game_id(ctx, GB_POKEGOLD_GAME.game_id);
#endif
    pokegold_init(ctx);
    int exit_code = 0;
