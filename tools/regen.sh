#!/usr/bin/env bash
#
# Regenerate the recompiled C sources in this repo from a fresh pret/pokegold
...
#
# What it does:
#   1. (Re)build pret/pokegold to produce pokegold.gb + pokegold.sym
...
#   5. Copy the generated pokegold_*.c / .h / manifest / metadata into this
...
if [[ $# -ne 2 ]]; then
    echo "Usage: $0 <pret/pokegold-path> <gb-recompiled-path>" >&2
    exit 1
fi
...
if [[ ! -d "$PRET_DIR" ]]; then
    echo "pret/pokegold directory not found: $PRET_DIR" >&2; exit 1
fi
...
# 1. Build the ROM.
echo "[regen] Building pokegold ROM..."
(cd "$PRET_DIR" && make pokegold)

ROM="$PRET_DIR/pokegold.gb"
SYM="$PRET_DIR/pokegold.sym"
for f in "$ROM" "$SYM"; do
...
if [[ -n "$BSS_TOOL" ]]; then
    ...
    python3 "$BSS_TOOL" "$OUT_DIR/pokegold_rom.c" "$OUT_DIR/pokegold.c"
fi

# 5. Copy back. Keep launcher_main.cpp and CMakeLists.txt — those are repo-
#    level, not generated.
echo "[regen] Copying generated sources into repo..."
cp -v "$OUT_DIR/pokegold_main.c" "$REPO_DIR/"
cp -v "$OUT_DIR/pokegold.c" "$REPO_DIR/"
cp -v "$OUT_DIR/pokegold_rom.c" "$REPO_DIR/"
cp -v "$OUT_DIR/pokegold_dispatch_chunk_"*.c "$REPO_DIR/"
cp -v "$OUT_DIR/pokegold_funcs_"*.c "$REPO_DIR/"
cp -v "$OUT_DIR/pokegold.h" "$REPO_DIR/"
cp -v "$OUT_DIR/pokegold_internal.h" "$REPO_DIR/"
cp -v "$OUT_DIR/assets_manifest_pokegold.h" "$REPO_DIR/"
cp -v "$OUT_DIR/pokegold_metadata.json" "$REPO_DIR/" 2>/dev/null || true
